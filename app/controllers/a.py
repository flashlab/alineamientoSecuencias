...
def generateAlignment():
    ...
    for i in range(0, alength, 60):
        ...
        list.append('{0:<4}  {1}  {2:<4}'.format(i + 1, seq1_slice, i + len(seq1_slice)))
        # list.append('                {0}'.format(alignment_str[i:i+60]))
        seq2_slice = seq2_aligned[i:i+60]
        list.append('{0:<4}  {1}  {2:<4}'.format(i + 1, seq2_slice, i + len(seq2_slice)))
        list.append("")
    return list
...
def testAlign2(seqA, seqB):

        global seq1, seq2
        seq1 = seqA
        seq2 = seqB
        list = generateAlignment()
        # rows = len(seq1) + 1
        # cols = len(seq2) + 1
        #
        # max_pos = create_score_matrix(rows, cols)
        print (list)
        return list
