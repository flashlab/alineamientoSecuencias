...
class QuerySelectField(SelectFieldBase):
    ...
    models instance, not the ID. Submitting a choice which is not in the query
    will result in a validation error.

    This field only works for queries on models whose primary key column(s)
    have a consistent string representation. This means it mostly only works
    for those composed of string, unicode, and integer types. For the most
    part, the primary keys will be auto-detected from the models, alternately
    ...
    a string, this is the name of an attribute on the models object to use as
    the label text. If a one-argument callable, this callable will be passed
    models instance and expected to return the label text. Otherwise, the models
...
class QuerySelectMultipleField(QuerySelectField):
    ...
    models instances and will be an empty list when no value is selected.
