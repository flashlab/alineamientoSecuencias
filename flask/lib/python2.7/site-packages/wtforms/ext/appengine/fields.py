...
class ReferencePropertyField(fields.SelectFieldBase):
    ...
    :param get_label:
        If a string, use this attribute on the models class as the label
        associated with each option. If a one-argument callable, this callable
        will be passed models instance and expected to return the label text.
        Otherwise, the models object's `__str__` or `__unicode__` will be used.
    :param allow_blank:
