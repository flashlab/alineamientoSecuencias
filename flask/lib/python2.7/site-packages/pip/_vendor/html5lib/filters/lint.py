        ...
        for token in _base.Filter.__iter__(self):
            ...
            if type in ("StartTag", "EmptyTag"):
                ...
                if contentModelFlag != "PCDATA":
                    raise LintError("StartTag not in PCDATA content models flag: %(tag)s" % {"tag": name})
                ...
                if contentModelFlag != "PCDATA":
                    raise LintError("Comment not in PCDATA content models flag")
                ...
                name = token["name"]
                if contentModelFlag != "PCDATA":
                    raise LintError("Doctype not in PCDATA content models flag: %(name)s" % {"name": name})
                if not isinstance(name, str):
                    raise LintError("Tag name is not a string: %(tag)r" % {"tag": name})
